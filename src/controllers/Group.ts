...
export function getGroup(id: number | string): Promise<Group> {
    return new Promise<Group>(async (resolve, reject) => {
        ...
        if (!checkID.test(id.toString())) return reject(new Error("Invalid Group ID."));
...
export function getAuditLogs(id: number | string, config?: {
    sort?: "Asc" | "Desc",
    limit?: 10 | 25 | 50 | 100,
    ...
    return new Promise<Logs>(async (resolve, reject) => {
        const checkID = new RegExp('^[0-9]+$');
        if (!checkID.test(id.toString())) return reject(new Error("Invalid Group ID.")); if (config?.limit && ![10, 25, 50, 100].includes(config?.limit)) return reject(new Error("The allowed limit values include: 10, 25, 50, and 100."));
        if (config?.sort && !["asc", "desc"].includes(config?.sort.toLowerCase())) return reject(new Error("The allowed sort values are Asc or Desc."));
        if (config?.actionType && !types.includes(config.actionType)) return reject(new Error(`The allowed actionType values are: ${types.join(", ")}`));
...
export function getGroupWall(id: number | string, config?: {
    sort?: "Asc" | "Desc",
    limit?: 10 | 25 | 50 | 100,
    cursor?: string
}): Promise<GroupWall> {
    return new Promise<GroupWall>(async (resolve, reject) => {
        const checkID = new RegExp('^[0-9]+$');
        if (!checkID.test(id.toString())) return reject(new Error("Invalid Group ID.")); if (config?.limit && ![10, 25, 50, 100].includes(config.limit)) return reject(new Error("The allowed limit values include: 10, 25, 50, and 100."));
        if (config?.sort && !["asc", "desc"].includes(config.sort.toLowerCase())) return reject(new Error("The allowed sort values are Asc or Desc."));
...
export function exileUser(groupId: number | string, userId: number | string): Promise<ExileUser> {
    return new Promise<ExileUser>(async (resolve, reject) => {
        const checkID = new RegExp('^[0-9]+$');
        if (!checkID.test(userId.toString())) return reject(new Error("Invalid User ID."));
        if (!checkID.test(groupId.toString())) return reject(new Error("Invalid Group ID"));
...
export function getGroupJoinRequests(id: number | string, config?: {
    sort?: "Asc" | "Desc",
    limit?: 10 | 25 | 50 | 100,
    cursor?: string
}): Promise<GroupJoinRequests> {
    return new Promise<GroupJoinRequests>(async (resolve, reject) => {
        const checkID = new RegExp('^[0-9]+$');
        if (!checkID.test(id.toString())) return reject(new Error("Invalid Group ID."));
        if (config?.limit && ![10, 25, 50, 100].includes(config.limit)) return reject(new Error("The allowed limit values include: 10, 25, 50, and 100."));
        if (config?.sort && !["asc", "desc"].includes(config.sort.toLowerCase())) return reject(new Error("The allowed sort values are Asc or Desc."));
